    ...
    ({ id, label }, ref) => {
      ...
      return (
        <a
          ...
          onClick={(event) => onNavLinkClick(event, id)}
          aria-current={isActive ? "page" : undefined}
          className={`relative w-full text-left py-6 px-6 border-b border-white/5 transition-all duration-300 flex items-center justify-between overflow-hidden group ${isActive ? 'bg-white/5' : 'hover:bg-white/5'
            }`}
        >
          <span className={`relative z-10 text-base md:text-lg uppercase tracking-widest transition-colors duration-300 ${isActive ? 'text-accent font-bold' : 'text-white/70 group-hover:text-white'
            }`}>
            {label}
          </span>
        ...
        >
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <Image
              ...
              width={48}
              height={48}
              className={`transition-all duration-700 flex-shrink-0 ${isScrolled
            ...
            <span className={`font-serif font-bold text-white transition-all duration-700 tracking-wide whitespace-nowrap ${isScrolled
